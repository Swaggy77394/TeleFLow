...
def apply_replace_with_entities(text, entities, find, replace):
    """
    ...
    while True:
        ...
        for e in new_entities:
            ...
            if e.offset >= pos + find_utf16_len:
                e.offset += diff
            elif e.offset <= pos and entity_end >= pos + find_utf16_len:
                e.length += diff
            elif e.offset > pos and entity_end < pos + find_utf16_len:
                e.offset = min(e.offset, pos + replace_utf16_len)
            
        text = text[:pos] + replace + text[pos + find_utf16_len:]
        curr_index = pos + replace_utf16_len
...
def apply_regex_with_entities(text, entities, pattern, replacement):
    ...
    # Process from right to left so index shifts do not affect matches to the left
...
def apply_text_transformations(original_text, original_entities, replacements, regex_active,
    ...
    Smart entity strategy (mirrors test4.py approach):
    - We preserve all original entities (bold, italic, links, premium custom emoji).
    - As plain replacements and regex rules are applied, we dynamically adjust the offsets and lengths
      of the entities so that they stay perfectly aligned with the transformed text body.
    - Finally, we parse prefix/suffix markdown and shift/merge everything together.
    """
    transformed_text = original_text
    transformed_entities = list(original_entities) if original_entities else []

    # Priority 1: plain str.replace
    ...
    # Parse prefix/suffix separately to preserve original/transformed entities exactly!
    clean_prefix, prefix_entities = markdown.parse(prefix)
    clean_suffix, suffix_entities = markdown.parse(suffix)
    
    final_clean_text = clean_prefix + transformed_text + clean_suffix
    
    # Merge all three entity lists
    merged_entities = []
    
    # 1. Prefix entities (offsets are already correct)
    if prefix_entities:
        merged_entities.extend(prefix_entities)
        
    # 2. Transformed original entities (need to be shifted by the length of the clean prefix)
    if transformed_entities:
        shift = utf16_len(clean_prefix)
        shifted_orig = shift_entities(transformed_entities, shift)
        if shifted_orig:
            merged_entities.extend(shifted_orig)
            
    # 3. Suffix entities (need to be shifted by the length of the clean prefix + transformed_text)
    if suffix_entities:
        shift = utf16_len(clean_prefix) + utf16_len(transformed_text)
        shifted_suffix = shift_entities(suffix_entities, shift)
        if shifted_suffix:
            merged_entities.extend(shifted_suffix)
