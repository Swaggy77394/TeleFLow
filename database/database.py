...
def init_db():
    """Initializes SQLite database schema and establishes MongoDB connection if configured."""
    global mongo_client, mongo_db, is_mongo_active
    
    # 1. Initialize SQLite first (always need schema for migration checks)
    os.makedirs(os.path.dirname(DB_PATH), exist_ok=True)
    sqlite_conn = sqlite3.connect(DB_PATH)
    sqlite_cursor = sqlite_conn.cursor()
    
    # Run SQLite creation SQL
    ...
    # 2. Check if MongoDB should be used
    if MONGO_URI and pymongo:
        try:
            ...
            # Run automatic one-time migration if needed
            migrate_sqlite_to_mongodb()
        except Exception as e:
            logger.error(f"Failed to connect to MongoDB. Falling back to SQLite. Error: {e}")
            is_mongo_active = False
    else:
        if MONGO_URI and not pymongo:
            logger.warning("MONGO_URI set but pymongo library not available. Using SQLite fallback.")
...
def restore_db_from_json(backup_data: dict) -> dict:
    ...
    if is_mongo_active:
        ...
        try:
            ...
            for r in rows:
                c.execute(
                    "INSERT OR IGNORE INTO replacements (source_id, target_id, find_text, replace_text) VALUES (?,?,?,?)",
                    (r["source_id"], r.get("target_id"), r["find_text"], r["replace_text"])
                )
            ...
            for r in rows:
                c.execute(
                    "INSERT OR IGNORE INTO header_footer (source_id, target_id, header, footer) VALUES (?,?,?,?)",
                    (r["source_id"], r["target_id"], r.get("header"), r.get("footer"))
                )
...
def get_replacements(source_id: int, target_id: int):
    if is_mongo_active:
        try:
            ...
            cursor = mongo_db.replacements.find(query, {"_id": 0, "find_text": 1, "replace_text": 1})
            return [(doc["find_text"], doc["replace_text"]) for doc in cursor]
        except Exception as e:
...
def get_header_footer(source_id: int, target_id: int):
    if is_mongo_active:
        try:
            doc = mongo_db.header_footer.find_one({"source_id": source_id, "target_id": target_id}, {"_id": 0, "header": 1, "footer": 1})
            if doc:
                return doc.get("header"), doc.get("footer")
            return None, None
        except Exception as e:
